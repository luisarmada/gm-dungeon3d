...
for(i = 1; i < array_length_2d(messageGiver.myDialogue, index1); ++i) {
    ...
    choiceBox[i - 1] = instance_create(view_xview + (view_wview / 2), (view_yview + (view_hview / 2)-100)+(i * 40), objChoiceBox); //Create the choiceBox and set it in the array
    //With allows us quick access to choiceBox
    with(choiceBox[i - 1]) {
        maxLength = sprite_width; //Set the max length of the message
        maxHeight = sprite_height; //Sett he max height of the message
